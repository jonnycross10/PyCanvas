...
class Courses(object):
	...
	def classData(self):
		...
		r = requests.get(self.url+"courses", headers = headers, params=options)
		...
		def currentClasses(d):
			...
			def thisTerm(d): #return classes from this term
				...
				for i in range(0,len(d)):
					startDate = dateutil.parser.parse(d[i]["created_at"])
	...
	#maybe use kwargs** to let user specify parameters
	def upcomingAssignments(self):
		assignments = self.allAssignments()
		notDueYet = {}
		for i in range(0,len(assignments)):
			for j in range(0,len(assignments[i])):
				notDueYet[assignments[i][j]['name']] = assignments[i][j]
		return notDueYet
		#if kwargs is name then return notDueYet.keys()
